...
export default async function ConcertCalendarPage({ searchParams }: Props) {
  ...
  const selectedConcertsSection = (
    <section className="panel p-6 md:p-8">
      <h2 className="section-title text-[1.35rem] md:text-[1.55rem]">{selectedDate ? `${selectedDate} の演奏会` : '日付を選択してください'}</h2>

      {dayError && <p className="mt-5">日付別一覧の取得に失敗しました。</p>}
      {!dayError && selectedDate && (selectedDayConcerts?.length ?? 0) === 0 && <p className="mt-5">この日の演奏会はありません。</p>}
